...
%for sessionID = [5]
    ...
    for t = 1:length(tags)
        ...
        for region = [1:10] %brain group I'm interested in
            tic
            d = [];
            d2 = [];
            dev_nr_all = [];
            ...
            for tt = 1:length(time_period_beginning)
                ...
                if ~isempty(neuron_in_BLA)
                    ...
                    [mdl0_reverse, FitInfo] = lassoglm(spikesTrial,h);
                    ...
                    for k = 1:length(ft)/20-1
                        ...
                        [mdl0_reverse_rotated, FitInfo2] = lassoglm(shifted, h);
            ...
            if ~isempty(neuron_in_BLA)
                ...
                title(['ability of model to predict Q1; data from ', num2str(region), ' ', subject, ' ', date, ' ', probeTag{1}])
            
           
             
            savefig(fullfile('C:\Users\annaL\Documents\PhD\value_coding\regions\Q', sprintf('%s_%s_%s_%s_region.fig',subject, date, probeTag{1}, num2str(region))))
            end
            toc
        end
